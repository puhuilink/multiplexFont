      ...
      >
        <a-col :span="24">
          <a-form-model-item
            ...
            :rules="[{ required: true, message: '时间范围必填', trigger: 'change' }]"
            prop="timeList"
          >
            <a-range-picker style="width: 300px" format="YYYY/MM/DD HH:mm" v-model="formData.timeList"></a-range-picker>
          </a-form-model-item>
        </a-col>
        <a-col :span="24">
          <a-form-model-item
    ...
    async onSubmit () {
      ...
      const data = await xungeng.request({
        ...
        if (row.type === 'application/octet-stream') {
          downloadFile('巡更报告--' + moment().format('YYYY-MM-DD HH:mm:ss'), row, { type: 'application/zip;charset=UTF-8' })
